...
const { loginLimit, loginMeLimit } = require('../middleware/apiLimiter');

const secretKey = process.env.SECRETKEY;

router.route('/').post(loginLimit, async (req, res) => {
    try {
        ...
        if (!data) return res.status(404).json({ error: 'User does not exist' });
...
router.route('/me').get(authyMiddlware, loginMeLimit, async (req, res) => {

    try {
        const id = req.user.id;
        const userP = await connect.query("SELECT id, username, email, profile_pic, bio FROM users WHERE id = $1", [id]);
